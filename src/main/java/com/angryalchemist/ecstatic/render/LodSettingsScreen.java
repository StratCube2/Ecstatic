...
public final class LodSettingsScreen extends Screen {
    ...
    protected void init() {
        ...
        double renderDistanceInitialValue = valueFromPercent(Math.round(this.config.lodRenderDistanceScale() * 100.0F), RENDER_DISTANCE_MIN_PERCENT, RENDER_DISTANCE_MAX_PERCENT);
        int renderDistanceInitialPercent = percentFromValue(renderDistanceInitialValue, RENDER_DISTANCE_MIN_PERCENT, RENDER_DISTANCE_MAX_PERCENT, SLIDER_STEP_PERCENT);
        AbstractSliderButton renderDistance = new AbstractSliderButton(
            centerX - 130, y, CHECKBOX_WIDTH, WIDGET_HEIGHT, Component.literal("Render distance: " + renderDistanceInitialPercent + "%"), renderDistanceInitialValue
        ) {
            protected void updateMessage() {
                this.setMessage(Component.literal("Render distance: " + LodSettingsScreen.percentFromValue(this.value, RENDER_DISTANCE_MIN_PERCENT, RENDER_DISTANCE_MAX_PERCENT, SLIDER_STEP_PERCENT) + "%"));
            }

            protected void applyValue() {
                LodSettingsScreen.this.config.setLodRenderDistanceScale(LodSettingsScreen.percentFromValue(this.value, RENDER_DISTANCE_MIN_PERCENT, RENDER_DISTANCE_MAX_PERCENT, SLIDER_STEP_PERCENT) / 100.0F);
            }
        };
        renderDistance.visible = y >= viewportTop && y + WIDGET_HEIGHT <= viewportBottom;
        ...
        workerThreads.visible = y >= viewportTop && y + WIDGET_HEIGHT <= viewportBottom;
        ...
        y += ROW_STRIDE;
        double fogFalloffInitialValue = valueFromPercent(Math.round(this.config.fogFalloffScale() * 100.0F), FOG_FALLOFF_MIN_PERCENT, FOG_FALLOFF_MAX_PERCENT);
        int fogFalloffInitialPercent = percentFromValue(fogFalloffInitialValue, FOG_FALLOFF_MIN_PERCENT, FOG_FALLOFF_MAX_PERCENT, SLIDER_STEP_PERCENT);
        AbstractSliderButton fogFalloff = new AbstractSliderButton(
            centerX - 130, y, CHECKBOX_WIDTH, WIDGET_HEIGHT, Component.literal("Fog falloff: " + fogFalloffInitialPercent + "%"), fogFalloffInitialValue
        ) {
            protected void updateMessage() {
                this.setMessage(Component.literal("Fog falloff: " + LodSettingsScreen.percentFromValue(this.value, FOG_FALLOFF_MIN_PERCENT, FOG_FALLOFF_MAX_PERCENT, SLIDER_STEP_PERCENT) + "%"));
            }

            protected void applyValue() {
                LodSettingsScreen.this.config.setFogFalloffScale(LodSettingsScreen.percentFromValue(this.value, FOG_FALLOFF_MIN_PERCENT, FOG_FALLOFF_MAX_PERCENT, SLIDER_STEP_PERCENT) / 100.0F);
            }
        };
        fogFalloff.visible = y >= viewportTop && y + WIDGET_HEIGHT <= viewportBottom;
        this.addRenderableWidget(fogFalloff);
        y += ROW_STRIDE;
        double fogIntensityInitialValue = valueFromPercent(Math.round(this.config.fogIntensity() * 100.0F), FOG_INTENSITY_MIN_PERCENT, FOG_INTENSITY_MAX_PERCENT);
        int fogIntensityInitialPercent = percentFromValue(fogIntensityInitialValue, 0, FOG_INTENSITY_MAX_PERCENT, SLIDER_STEP_PERCENT);
        AbstractSliderButton fogIntensity = new AbstractSliderButton(
            centerX - 130, y, CHECKBOX_WIDTH, WIDGET_HEIGHT, Component.literal("Fog intensity: " + fogIntensityInitialPercent + "%"), fogIntensityInitialValue
        ) {
            protected void updateMessage() {
                this.setMessage(Component.literal("Fog intensity: " + LodSettingsScreen.percentFromValue(this.value, FOG_INTENSITY_MIN_PERCENT, FOG_INTENSITY_MAX_PERCENT, SLIDER_STEP_PERCENT) + "%"));
            }

            protected void applyValue() {
                LodSettingsScreen.this.config.setFogIntensity(LodSettingsScreen.percentFromValue(this.value, FOG_INTENSITY_MIN_PERCENT, FOG_INTENSITY_MAX_PERCENT, SLIDER_STEP_PERCENT) / 100.0F);
            }
        };
        fogIntensity.visible = y >= viewportTop && y + WIDGET_HEIGHT <= viewportBottom;
        ...
        Button debugButton = Button.builder(Component.literal("Debug Settings..."), b -> Minecraft.getInstance().setScreen(new LodDebugSettingsScreen(this)))
            .bounds(centerX - 130, y, CHECKBOX_WIDTH, WIDGET_HEIGHT)
            .build();
        debugButton.visible = y >= viewportTop && y + WIDGET_HEIGHT <= viewportBottom;
        this.addRenderableWidget(debugButton);
        int footerY = this.height - FOOTER_HEIGHT + WIDGET_HEIGHT;
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.maxScroll > 0) {
            ...
            int trackTop = 50;
            int trackBottom = this.height - FOOTER_HEIGHT;
            int trackHeight = trackBottom - trackTop;
            guiGraphics.fill(trackX, trackTop, trackX + SCROLLBAR_WIDTH, trackBottom, 1090519039);
            ...
            guiGraphics.fill(trackX, thumbY, trackX + SCROLLBAR_WIDTH, thumbY + thumbHeight, -4144960);
        }
    }
